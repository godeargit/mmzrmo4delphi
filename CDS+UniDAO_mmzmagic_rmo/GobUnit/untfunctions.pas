...
//    FConn: TADOConnection;
  ...
  public
    ...
    constructor Create(IConStr: string; iTimeOut: integer = 15; ICreateBuffCount:
      Integer = 5); overload;
    constructor Create(ICreateBuffCount: Integer = 5); overload;
    destructor Destroy; override;
    {获取一个ADO对象 可以指定名字 如果没有名字 系统自己返回一合适的对象}
    function GetAnQuery(IuserTime: integer = 1; Iname: string = ''): TClientDataSet; overload;
    function GetAnQuery(Iname: string): TClientDataSet; overload;
    {获取自动增长的ID号码}
    ...
    {用指定的ＡＤＯ执行}
    function OpenDataset(IadoName, ISql: string): TClientDataSet; overload;
    ...
    {释放ADO使用权以便其它人员使用}
    procedure BackToPool(Iado: TClientDataSet); overload;
    procedure BackToPool(IName: string); overload;
    {加入一个由外部创建的ADO 帮它管理生命周期和重用}
    procedure AddAnOutAdo(Iado: TClientDataSet);
    {为操作表预备一个ADO}
    function Ready(ItabName: string; Iado: TClientDataSet): TClientDataSet; overload;
    function Ready(ItabName: string; IQueryRight: integer = 1): TClientDataSet; overload;
    {打开一个表}
    ...
    {检查是否处于可修改状态}
    function CheckModState(IAdo: TClientDataSet): boolean;
    {安全保存}
    function SafePost(Iado: TClientDataSet): boolean;
    {查询总共有多少个ADOquery}
...
constructor TDBMrg.Create(ICreateBuffCount: Integer = 5);
var
  I: Integer;
begin
  FautoFree := true;
...
{-------------------------------------------------------------------------------
  ...
  作者:      马敏钊
  日期:      2006.01.11
  参数:      Iado: TClientDataSet
  返回值:    无
  说明:      加入一个由外部创建的ADO 帮它管理生命周期和重用
-------------------------------------------------------------------------------}

procedure TDBMrg.AddAnOutAdo(Iado: TClientDataSet);
begin
  Iado.Close;
...
begin
  for I := 0 to FPool.Count - 1 do begin // Iterate
    if TClientDataSet(FPool.Objects[i]).Name = IName then begin
      FPool.Strings[i] := CDb_State_NoneUsed;
    end;
  end; // for
end;

procedure TDBMrg.BackToPool(Iado: TClientDataSet);
begin
  if Iado = nil then
    Exit;
  try
...
begin
  with GetAnQuery(CDb_State_CanUsed) do begin
    try
      Close;
      CommandText := Format('Delete from %s where %s=:VarIant', [ItabName, IFieldName]);
      Params.ParamValues['VarIant'] := Ivalue;
      Execute;
    finally
      Close;
    end;
...
function TDBMrg.GetAnQuery(Iname: string): TClientDataSet;
var
  I: Integer;
begin
  Result := nil;
  ...
  if Iname <> '' then begin
    for I := 0 to FPool.Count - 1 do
      if TClientDataSet(FPool.Objects[i]).Name = 'MyPool' + Iname then begin
        Result := TClientDataSet(FPool.Objects[i]);
        Exit;
      end;
...
begin
  with GetAnQuery do begin
    Close;
    CommandText := Format('Select Count(%s) as MyCount from %s where %s=:variant',
...
begin
  ...
  with GetAnQuery do begin
    Close;
    CommandText := Format('Select Max(%s) as myMax  from  %s', [IFieldName, ItabName]);
    Open;
...
begin
  with GetAnQuery(CDb_State_CanUsed) do begin
    try
      Close;
      CommandText := Format('Select %s as MyGetField from %s where %s=:VarIant', [IGetField, ItabName, IWHereField]);
      Params.ParamValues['VarIant'] := Ivalue;
      Open;
...
function TDBMrg.OpenDataset(ISql: string): TClientDataSet;
begin
  Result := GetAnQuery;
...
function TDBMrg.OpenDataset(IadoName, ISql: string): TClientDataSet;
begin
...
begin
  Result := 0;
  for I := 0 to FPool.Count - 1 do
    if TClientDataSet(FPool.Objects[i]).IsEmpty then
      Inc(Result);
end;
...
begin
  Result := false;
  with TClientDataSet(IData) do begin
...
begin
  with GetAnQuery(CDb_State_CanUsed) do begin
    Close;
    CommandText := Format('Select Count(*) as MyCount from %s', [ItabName]);
    Open;
    Result := Fieldbyname('MyCount').AsInteger;
  end; // with
end;
...
{-------------------------------------------------------------------------------
  ...
  作者:      马敏钊
  日期:      2006.02.21
  参数:      ItabName:string;Iado:TClientDataSet
  返回值:    无
  说明:      为操作表预备一个ADO
-------------------------------------------------------------------------------}

function TDBMrg.Ready(ItabName: string; Iado: TClientDataSet): TClientDataSet;
begin
  with Iado do begin
    Close;
    CommandText := Format('Select * from %s where 1=2', [ItabName]);
    Open;
  end; // with
  Result := Iado;
end;

function TDBMrg.Ready(ItabName: string; IQueryRight: integer = 1): TClientDataSet;
begin
  Result := GetAnQuery(IQueryRight);
  with TClientDataSet(Result) do begin
    Close;
    CommandText := Format('Select * from %s where 1=2', [ItabName]);
    Open;
  end; // with
...
function OpenDataset(Iado: TClientDataSet; ISql: string):
  TClientDataSet; overload;
begin
  Result := Iado;
  with Result do begin
...
function TDBMrg.OpenDataset(Iado: TClientDataSet; ISql: string; const Args: array of
  const): TClientDataSet;
begin
  ISql := Format(Isql, Args);
  Result := Iado;
  with Result do begin
...
function TDBMrg.ExecAnSql(Iado: TClientDataSet; Isql: string; const Args: array of const): Integer;
begin
  Isql := Format(Isql, Args);
  with Iado do begin
    try
...
function TDBMrg.OpenTable(ItabName: string; Iado: TClientDataSet): TClientDataSet;
begin
  with Iado do begin
    Close;
    CommandText := Format('Select * from %s ', [ItabName]);
    Open;
  end; // with
  Result := Iado;
end;

function TDBMrg.CheckModState(IAdo: TClientDataSet): boolean;
begin
  Result := IAdo.State in [dsEdit, dsinsert];
end;

function TDBMrg.SafePost(Iado: TClientDataSet): boolean;
begin
...
function TDBMrg.OpenTable(ItabName: string; IQueryRight: integer = 1): TClientDataSet;
begin
  Result := GetAnQuery(IQueryRight);
  with TClientDataSet(Result) do begin
    Close;
...
function TDBMrg.OpenDataset(Iado: TClientDataSet; ISql: string): TClientDataSet;
begin
  Result := Iado;
  with Result do begin
    Close;
